...
# Pages with chunk counts outside this range will be flagged and excluded.
MIN_CHUNKS = 4
MAX_CHUNKS = 7
...
def chunk_page_structurally(url: str, session: requests.Session) -> list[dict]:
    ...
    try:
        ...
        print(f"    [+] Found {len(chunks)} potential chunks from this page.")
        return chunks
...
def main():
    ...
    # The help text here updates automatically based on the constants defined above.
    ...
    with open(args.review_file, 'w', encoding='utf-8', newline='') as review_file:
        ...
        for i, url in enumerate(urls_to_process, 1):
            ...
            if verify_page_category(url, session):
                ...
                # --- NEW LOGIC ---
                # Check if the number of chunks is within the valid range.
                if MIN_CHUNKS <= num_chunks <= MAX_CHUNKS:
                    # If it is, accept the chunks and add them to our main list.
                    print(f"    [✓] ACCEPTED. {num_chunks} chunks fall within the {MIN_CHUNKS}-{MAX_CHUNKS} range.")
                    all_chunks.extend(page_chunks)
                else:
                    # If it's outside the range, flag it for review and DO NOT add the chunks.
                    print(f"    [!] DISCARDED & FLAGGED. {num_chunks} chunks is outside the {MIN_CHUNKS}-{MAX_CHUNKS} range.")
                    review_file.write(f"{url},{num_chunks}\n")
    
    print(f"\n[*] Curation complete. Total chunks accepted and saved: {len(all_chunks)}")
    print(f"[*] Saving all accepted chunks to '{args.output}'...")
    with open(args.output, 'w', encoding='utf-8') as f:
        json.dump(all_chunks, f, indent=2, ensure_ascii=False)
    
    print(f"\n[***] Curated knowledge base saved in '{args.output}'.")
    print(f"[***] URLs needing review (due to chunk count) have been saved to '{args.review_file}'. [***]")
