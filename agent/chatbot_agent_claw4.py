...
def execute_dual_track_search(query: str, search_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        search_params = {
            ...
            'keyword': query,
            'match_count': 12,  # Get more for better reranking
            'similarity_threshold': 0.5  # Lower threshold for more recall
        }
        ...
        if combined_results:
            ...
            try:
                ...
                rerank_response = co.rerank(
                    ...
                    query=query,
                    documents=documents,
                    top_n=min(8, len(documents)),  # Top 8 after reranking
    ...
    if len(queries) > 1 and deduplicated_chunks:
        ...
        try:
            final_rerank = co.rerank(
                ...
                query=main_query,
                documents=documents,
                top_n=min(12, len(documents)),  # Top 12 overall
                return_documents=True
            )
...
**CRITICAL RULES:**
- Answer questions only related to dementia care
- Put ALL sub-queries in one JSON array
- Always interpret any dementia-related statement (e.g. “I have FTD”, “My dad has Alzheimer's”) as an implicit request for information or support, and perform a database search based on possible concerns or needs.
...
<!-- Example below is ONLY for your reference, not part of the live chat
     ----------------------------------------------------------------- -->
**EXAMPLE:**

Question: How can I help my mom with eating problems and wandering at night?

Thought: I need to research eating problems in dementia, nighttime wandering prevention, and general behavioral management strategies. I'll search for all these topics in parallel.
Action: {tool_names}
Action Input: ["managing eating problems in dementia", "preventing nighttime wandering", "dementia behavioral management strategies"]
...
def ask_parallel_enhanced_chatbot(
    ...
    agent_executor = AgentExecutor(
        ...
        max_iterations=3  # Keep it simple
    )
